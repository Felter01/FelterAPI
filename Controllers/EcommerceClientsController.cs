...
public class EcommerceClientsController : ControllerBase
{
    ...
    {
        return Ok(await _ctx.EcommerceClients.AsNoTracking().ToListAsync());
    }

    [HttpGet("{id:guid}")]
    public async Task<IActionResult> GetById(Guid id)
    {
        var entity = await _ctx.EcommerceClients.FindAsync(id);
        if (entity == null) return NotFound();
        return Ok(entity);
    }

    [HttpPost]
    public async Task<IActionResult> Create([FromBody] EcommerceClient model)
    {
        model.Id = Guid.NewGuid();
        _ctx.EcommerceClients.Add(model);
        await _ctx.SaveChangesAsync();
        return CreatedAtAction(nameof(GetById), new { id = model.Id }, model);
    }

    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] EcommerceClient model)
    {
